...
public class Vector2D {
    ...
    public Vector2D getSub(Vector2D v){return new Vector2D(this.x-v.x,this.y-v.y);}
    ...
    public double getDotProduct(Vector2D v){
        return this.getLength()*v.getLength()*Math.cos(this.getAngle(v));
    }

    public double getAngle(Vector2D v){
        return (180/Math.PI)*Math.acos(this.dot(v)/(v.getLength()*this.getLength()));
    }
}